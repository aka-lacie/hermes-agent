...
### Kanban Persistent Worker Daemons
...
- Adds `hermes kanban worker --profile <profile>`, a headless non-gateway
  worker runtime that claims only tasks assigned to its configured profile.
- Persistent workers register in `kanban_worker_daemons` with PID, heartbeat,
  board, and loopback UDP wake port. Normal dispatchers defer ready/review work
  to a healthy matching daemon and use subprocess dispatch as fallback when no
  daemon is healthy.
- `kanban_create`/unblock/link nudges now wake registered worker daemons across
  processes as well as the in-process gateway dispatcher.
- The built-in worker keeps a daemon-local CLI/runtime host warm across
  back-to-back tasks while resetting session/conversation state and task env per
  claimed task.
- Gateway dispatcher logs include `deferred_to_daemon` counts so logs make the
  daemon-vs-fallback path visible.
- Config has a generic `kanban.worker_daemon` shape for future supervisor
  integration without hardcoded profile names.

Main files:
- `hermes_cli/kanban_worker.py`
- `hermes_cli/kanban_db.py`
- `hermes_cli/kanban.py`
- `gateway/kanban_dispatch_signal.py`
- `gateway/run.py`
- `hermes_cli/config.py`
- `tests/hermes_cli/test_kanban_worker_daemon.py`

Local setup:
- Installed user systemd template:
  `/home/sonya/.config/systemd/user/hermes-kanban-worker@.service`.
- Enabled and started:
  `hermes-kanban-worker@executor-general.service` and
  `hermes-kanban-worker@executor-codex.service`.
- Restarted `hermes-gateway-yuri.service` so its embedded dispatcher loaded the
  daemon deferral code.
- Verified canaries:
  `t_df8d706d` claimed by daemon PID `3589722` for `executor-general`, and
  `t_7e08c0e8` claimed by daemon PID `3589715` for `executor-codex`; both
  completed with persistent-worker logs.

Merge rule:
- Preserve the DB-backed worker daemon registry, loopback wake path, and
  dispatcher deferral bucket. Do not regress to a process-local-only nudge.
- Preserve profile-scoped claiming: a worker daemon must only dispatch tasks
  whose assignee matches its configured profile.
- Preserve subprocess fallback semantics when daemon rows are absent or stale.
- Preserve per-task isolation even when the daemon keeps CLI/tool/MCP runtime
  state warm.
- Keep the feature agent-name agnostic; local executor service names are
  workstation setup, not source-level constants.
