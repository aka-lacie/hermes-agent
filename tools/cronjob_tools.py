...
Agent jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered. Script jobs skip the LLM and deliver non-empty stdout. Reminder jobs emit the prompt without a detached agent and default to internal-turn delivery. delivery_mode='direct' posts output as-is; 'internal_turn' wakes the destination agent with conversation context. Cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
