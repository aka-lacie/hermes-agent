...
def test_create_stamps_session_id_from_gateway_context(monkeypatch, worker_env):
    """Gateway turns bind the conversation id through session ContextVars,
    which ``kanban_create`` should prefer over process-global env state."""
    monkeypatch.delenv("HERMES_SESSION_ID", raising=False)
    from gateway.session_context import clear_session_vars, set_session_vars
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb

    tokens = set_session_vars(session_id="gateway-sess-abc")
    try:
        out = kt._handle_create({
            "title": "from gateway context",
            "assignee": "peer",
            "parents": [worker_env],
        })
    finally:
    ...
    try:
        new_task = kb.get_task(conn, d["task_id"])
        assert new_task.session_id == "gateway-sess-abc"
    finally:
        conn.close()


def test_create_nudges_dispatcher_when_session_stamped(monkeypatch, worker_env):
    """Session-stamped gateway creates should wake the embedded dispatcher
    instead of waiting for the next periodic poll."""
    from gateway.session_context import clear_session_vars, set_session_vars
    import gateway.kanban_dispatch_signal as dispatch_signal
    from tools import kanban_tools as kt
    ...
    tokens = set_session_vars(session_id="gateway-sess-nudge")
    try:
        out = kt._handle_create({
            "title": "wake dispatcher",
            "assignee": "peer",
            "parents": [worker_env],
        })
    finally:
    ...
    assert calls == [{
        "task_id": d["task_id"],
        "board": None,
        "session_id": "gateway-sess-nudge",
    }]


def test_create_without_session_id_does_not_nudge_dispatcher(monkeypatch, worker_env):
    ...
    out = kt._handle_create({
        "title": "ordinary worker follow-up",
        "assignee": "peer",
        "parents": [worker_env],
    })
...
def test_comment_followup_subscribes_from_current_event_without_dispatch(
    monkeypatch, worker_env
):
    from gateway.session_context import clear_session_vars, set_session_vars
    import gateway.kanban_dispatch_signal as dispatch_signal
    ...
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="needs human note", assignee="peer")
    ...
    tokens = set_session_vars(
        platform="telegram",
        chat_id="chat-follow",
        thread_id="topic-follow",
        user_id="user-follow",
        session_id="gateway-sess-comment",
    )
    try:
        out = kt._handle_comment({
            "task_id": tid,
            "body": "use the newer acceptance criteria",
        })
    finally:
        clear_session_vars(tokens)
    ...
    try:
        ...
        conn.close()
    assert task.session_id == "gateway-sess-comment"
    assert len(subs) == 1
    assert subs[0]["chat_id"] == "chat-follow"
    assert subs[0]["last_event_id"] == current_max
    assert subs[0]["last_event_id"] > prior_max


def test_create_auto_subscribes_gateway_chat_context(monkeypatch, worker_env):
    """A kanban_create call from a chat-backed orchestrator subscribes the
    originating chat so the existing gateway notifier still emits pings."""
    ...
    tokens = set_session_vars(
        ...
        thread_id="thread-9",
        user_id="user-7",
        session_id="gateway-sess-sub",
    )
    try:
        out = kt._handle_create({
            "title": "subscribed child",
            "assignee": "peer",
            "parents": [worker_env],
        })
    finally:
...
def test_link_followup_subscribes_child_and_nudges_dispatcher(monkeypatch, worker_env):
    from gateway.session_context import clear_session_vars, set_session_vars
    import gateway.kanban_dispatch_signal as dispatch_signal
    from hermes_cli import kanban_db as kb
    ...
    conn = kb.connect()
    try:
        parent = kb.create_task(conn, title="finished parent", assignee="x")
        kb.complete_task(conn, parent, summary="parent done")
        child = kb.create_task(conn, title="existing child", assignee="x")
    finally:
        conn.close()
    ...
    tokens = set_session_vars(
        platform="telegram",
        chat_id="chat-link",
        thread_id="topic-link",
        user_id="user-link",
        session_id="gateway-sess-link",
    )
    try:
    ...
    try:
        ...
        conn.close()
    assert task.session_id == "gateway-sess-link"
    assert len(subs) == 1
    assert subs[0]["chat_id"] == "chat-link"
...
def test_unblock_followup_subscribes_and_nudges_dispatcher(monkeypatch, worker_env):
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    from gateway.session_context import clear_session_vars, set_session_vars
    import gateway.kanban_dispatch_signal as dispatch_signal
    from hermes_cli import kanban_db as kb
    ...
    try:
        tid = kb.create_task(
            ...
            assignee="worker",
            session_id="old-origin-session",
        )
        kb.block_task(conn, tid, reason="needs decision")
    finally:
        conn.close()
    ...
    tokens = set_session_vars(
        platform="telegram",
        chat_id="chat-unblock",
        thread_id="topic-unblock",
        user_id="user-unblock",
        session_id="gateway-sess-unblock",
    )
    try:
        out = kt._handle_unblock({"task_id": tid})
    finally:
    ...
    try:
        ...
        conn.close()
    assert task.status == "ready"
    assert task.session_id == "gateway-sess-unblock"
    assert len(subs) == 1
    assert subs[0]["chat_id"] == "chat-unblock"
