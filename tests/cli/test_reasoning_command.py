...
class TestInlineThinkBlockExtraction(unittest.TestCase):
    ...
    def _make_agent(self):
        ...
        agent._preserve_provider_data = AIAgent._preserve_provider_data.__get__(agent)
    ...
    def test_build_assistant_message_preserves_provider_data(self):
        agent = self._make_agent()
        api_msg = self._build_msg(
            "Visible answer.",
            reasoning="Structured reasoning from API.",
        )
        api_msg.provider_data = {
            "google": {
                "gemini_content": {
                    "role": "model",
                    "parts": [
        ...
        result = agent._build_assistant_message(api_msg, "stop")
        self.assertIn("provider_data", result)
        self.assertEqual(
            result["provider_data"]["google"]["gemini_content"]["parts"][1]["thoughtSignature"],
            "sig-answer",
        )

    def test_prepare_api_message_keeps_provider_data_only_for_replay_routes(self):
        agent = self._make_agent()
        msg = {
            "role": "assistant",
        ...
        native_msg = agent._prepare_api_message(
            msg,
            preserve_provider_data=True,
            sanitize_tool_calls=False,
        )
        self.assertIn("provider_data", native_msg)
        self.assertNotIn("reasoning_content", native_msg)

        generic_msg = agent._prepare_api_message(
            msg,
            preserve_provider_data=False,
            sanitize_tool_calls=False,
        )
        self.assertNotIn("provider_data", generic_msg)
