...
def switch_model(
    ...
    def _resolve_runtime_provider_compat(
        provider: str,
        *,
        ...
        try:
            ...
            msg = str(e)
            if not any(name in msg for name in ("target_model", "explicit_api_key", "explicit_base_url")):
                raise
    ...
    if provider_changed or explicit_provider:
        ...
        if _user_pdef is not None and _user_pdef.base_url:
            ...
            try:
                runtime = _resolve_runtime_provider_compat(
                    target_provider,
            ...
            try:
                runtime = _resolve_runtime_provider_compat(target_provider)
        ...
        try:
            runtime = _resolve_runtime_provider_compat(current_provider)
    ...
    if not validation.get("accepted"):
        override = False
        if isinstance(user_providers, dict):
            provider_cfg = user_providers.get(target_provider)
            if isinstance(provider_cfg, dict):
                if new_model in _declared_model_ids(provider_cfg.get("models", {})):
                    override = True
        elif user_providers:
            for up in user_providers:
                if isinstance(up, dict) and up.get("provider") == target_provider:
                    if new_model in _declared_model_ids(up.get("models", [])):
                        override = True
                        break
